...
t_data <- read_csv("data/GLB.Ts+dSST.csv", skip = 1, na = "***") %>%
  ...
  drop_na() #this will remove any row with NA

annotation <- t_data %>% #to return the first and last value with an algorithm
  arrange(year) %>% #to be sure that the data is in the right order
  slice(1, n()) %>%
  mutate(t_diff = 0,
         x = year + c(-5,5)) #moves the labels to the sides

max_t_diff <- format(round(max(t_data$t_diff), 1), nsmall = 1)

t_data %>%
  ggplot(aes(x=year, y=t_diff, fill = t_diff)) +
  geom_col(show.legend = FALSE) + #geom_bar(includes a summary function build in, geom_col() plot the values as they are)
...
ggsave("figures/temperarure_bar_plot.png", width = 7, height = 4) 
