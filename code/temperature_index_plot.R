#based in https://youtu.be/fskblEWSeWU

# recreation of the graph in https://climate.nasa.gov/vital-signs/global-temperature/
...
read_csv("data/GLB.Ts+dSST.csv", skip = 1, na = "***") %>%
  ...
  scale_color_manual(name=NULL,
                     ...
                     guide= guide_legend(override.aes = list(shape=15, size = 5))) + #this replace the circles in legend for squares
  labs(
    x = "YEAR",
  ...
  theme(
    ...
    plot.title.position = "plot", #aligment of the tittle
    plot.title = element_text(margin = margin(b= 10), color = "red", face = "bold"),
    plot.subtitle = element_text(size = 8,margin = margin(b= 10)),
    legend.position = c(0.15, 0.9),
    legend.title = element_text(size = 0), #this is to delete the tittle of the legend
    legend.key.height = unit(10, "pt"),
    legend.margin = margin(0,0,0)
  )

ggsave("figures/temperarure_index_plot.png", width = 6, height = 4)
